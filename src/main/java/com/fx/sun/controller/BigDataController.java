 ...
 * @author pscha
 */
public class BigDataController implements Initializable, PopulateInterface {
    ...
    @FXML
    private ComboBox<ComboBoxPOJO> cbDays;
    @FXML
    ...
    public BigDataController(MainController mainController) {
    ...
    public void initialize(URL url, ResourceBundle bundle) {
        ...
        tfLat.setText(lat + "");
        tfLon.setText(lon + "");
        dpStart.setValue(now);
        dpEnd.setValue(now.plusDays(1));
        ...
        ArrayList cbDaysList = new ArrayList();
        cbDaysList.add(new ComboBoxPOJO(1, "1 Day"));
        cbDaysList.add(new ComboBoxPOJO(2, "2 Days"));
        cbDaysList.add(new ComboBoxPOJO(10, "10 Days"));

        cbDays.getItems().addAll(cbDaysList);
        cbDays.getSelectionModel().selectFirst();
        ...
        dpStart.valueProperty().addListener((ov, o, n) -> {
            populateTable();
        });

        dpEnd.valueProperty().addListener((ov, o, n) -> {
            populateTable();
        });

        cbDays.getSelectionModel().selectedItemProperty().addListener((ov, o, n) -> {
            populateTable();
        });
    ...
    private void populateTable() {
        ...
        start = dpStart.getValue();
        end = dpEnd.getValue();
        min = cbMin.getSelectionModel().getSelectedItem().getId();
        int days = cbDays.getSelectionModel().getSelectedItem().getId();

        list = new ArrayList<>();

        for (LocalDate sel = start; sel.isBefore(end); sel = sel.plusDays(days)) {
            for (int i = 0; i < 24; i++) {
                for (int j = 0; j < (60 / min); j++) {
                    Date date = Date.from(sel.atStartOfDay(ZoneId.systemDefault()).toInstant());
                    ...
                    LocalDateTime t = sel.atTime(i, j * min, 0);
                    ...
                    if (rbMoon.isSelected()) {
                        ...
                        TablePOJO tablePOJO = new TablePOJO(dateNow, time, azi, alti, dist, para, null);
                        list.add(tablePOJO);
                    } else {
                        ...
                        TablePOJO tablePOJO = new TablePOJO(dateNow, time, azi, alti, dist, null, trueAlti);
                        list.add(tablePOJO);
                    }
                }
            }
    ...
    private void exportCSV() {
        ...
        String dayStart = simpleDateFormat.format(calendar.getTime());

        Date dateEnd = Date.from(end.atStartOfDay(ZoneId.systemDefault()).toInstant());
        calendar = Calendar.getInstance();
        calendar.setTime(dateEnd);

        String dayEnd = simpleDateFormat.format(calendar.getTime());

        String obj = rbMoon.isSelected() ? bundle.getString("moon") : bundle.getString("sun");
        String resDays = "Days Res " + cbDays.getSelectionModel().getSelectedItem().getName() + "";
        String res = "Time Res " + cbMin.getSelectionModel().getSelectedItem().getName() + "";

        FileChooser fileChooser = new FileChooser();
        String fileName = obj + " " + dayStart + " - " + dayEnd + " " + resDays + " " + res + ".csv";
